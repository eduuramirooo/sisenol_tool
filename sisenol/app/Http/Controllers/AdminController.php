...

{
    ...
    {
        $usuarioId = session('id'); // este es el id del usuario, no el id de la sesión
...
public function actualizarDocumento(Request $request)
{
    $request->validate([
        ...
        'documento' => 'required|mimes:pdf|max:5120' // hasta 5 MB
    ]);
    ...
    $rutaDestino = public_path('upload/document/' . $nombreProducto);
...
public function crearProducto(Request $request)
{
    try {
        ...
        if ($request->hasFile('imagen')) {
            ...
            // Log para comprobar nombre y ruta
